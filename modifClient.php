...
if(!empty($_POST)){
      $query="update clients set nom=:nom,ville=:ville,telephone=:tel where idclients=:id";
      $pdostmt=$pdo->prepare($query);
      $pdostmt->execute(["nom"=>$_POST["inputnom"],"ville"=>$_POST["inputville"],"tel"=>$_POST["inputtel"],"id"=>$_POST["myid"]]);
      header("Location:clients.php");
}
if(!empty($_GET["id"])){
        $query="select * from clients where idclients=:id";
        $pdostmt=$pdo->prepare($query);
...
<form class="row g-3" method="POST">
    <input type="hidden" name="myid" value="<?php echo $row["idclients"] ?>"/>
  <div class="col-md-6">
